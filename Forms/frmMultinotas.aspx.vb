...
Partial Class frmMultinotas
    ...
    Private Sub obtenerIDDefinicionDoc(ByVal p_idmul As Integer)
        Dim dt As New DataTable
        Dim obj(1) As Object
        obj(0) = p_idmul
        obj(1) = ""
        dt = OracleHelper.ExecuteDataset(cad1, "doc_laboral.pr_multinota_get_iddefdoc", obj).Tables(0)
        If dt.Rows.Count > 0 Then
            Boxmulidrel.Value = dt.Rows(0)("iddefiniciondocrel")
        End If
    End Sub
    ...
    Protected Sub btnnuevo_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles btnnuevo.Click
        ...
        panelmultinota.Visible = False

        boxaccion.Value = acciones.Insertar
        Dim nromul As Integer
        nromul = OracleHelper.ExecuteScalar(cad1, CommandType.Text, "select doc_laboral.seq_multinotas.nextval  from dual")
    ...
    Protected Sub OnConfirm(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
    ...
    Public Sub obtenernotas_all()
        Try
            ...
            obj(1) = ""
            dt = OracleHelper.ExecuteDataset(cad1, "doc_laboral.multinota_select_all_xcta", obj).Tables(0)
            GrillaMultinota.DataSource = dt
            GrillaMultinota.DataBind()
            If dt.Rows.Count > 0 Then
                For i = 0 To dt.Rows.Count - 1

                    If BuscarAdjutnos(CInt(Session("nrocuenta")), CInt(GrillaMultinota.Rows(i).Cells(0).Text)).Rows.Count > 0 Then
                        CType(GrillaMultinota.Rows(i).Cells(7).FindControl("imgadjunto"), ImageButton).Visible = True
                    Else
                        CType(GrillaMultinota.Rows(i).Cells(7).FindControl("imgadjunto"), ImageButton).Visible = False
                    End If


                    If dt.Rows(i)("abandeja") = 0 Then
                        CType(GrillaMultinota.Rows(i).Cells(8).FindControl("btnenviar"), Button).Enabled = True
                        CType(GrillaMultinota.Rows(i).Cells(7).FindControl("imgadjunto"), ImageButton).Enabled = True
                        CType(GrillaMultinota.Rows(i).Cells(7).FindControl("btnmodeloeditar"), ImageButton).Enabled = True

                    Else
                        CType(GrillaMultinota.Rows(i).Cells(8).FindControl("btnenviar"), Button).Enabled = False
                        CType(GrillaMultinota.Rows(i).Cells(7).FindControl("imgadjunto"), ImageButton).Enabled = False
                        CType(GrillaMultinota.Rows(i).Cells(7).FindControl("btnmodeloeditar"), ImageButton).Enabled = False
                    End If

                Next

                PanelEditor.Visible = False
                panelmultinota.Visible = True

            Else
                GrillaMultinota.DataSource = Nothing
                GrillaMultinota.DataBind()
    ...
    Protected Sub btnenviar_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        idMul = Convert.ToInt32(row.Cells(0).Text)
        boxidmul.Value = idMul
        boxmes.Value = Convert.ToInt32(row.Cells(3).Text)
        boxano.Value = Convert.ToInt32(row.Cells(4).Text)
        ...
        obtenerIDDefinicionDoc(idMul)

        Session("mulidrel") = Boxmulidrel.Value

        btn.Enabled = False
        CType(row.Cells(6).FindControl("imgadjunto"), ImageButton).Enabled = False

        Dim popupScript4 As String
        popupScript4 = String.Empty

        popupScript4 = "<script language='JavaScript'>"
        popupScript4 += "window.open('frmReportemultinota.aspx?coddel=" + ddlDelegacion.SelectedValue + "&acc=0', 'CustomPopUp2', "
    ...
    Protected Sub imgadjunto_Click(ByVal sender As Object, ByVal e As System.Web.UI.ImageClickEventArgs)
        Dim btn As ImageButton = CType(sender, ImageButton)
        Dim row As GridViewRow = CType(btn.NamingContainer, GridViewRow)
        Dim idMul As Integer
        idMul = Convert.ToInt32(row.Cells(0).Text)
    ...
    Public Function EstaEnBandejaMultinota(ByVal pnrocuenta As Integer, ByVal pidmul As Integer) As Integer
        ...
        Try
            ...
            comando.CommandType = CommandType.StoredProcedure
            comando.Connection = conexion
            comando.CommandText = "doc_laboral.F_EnbandejaMultinota"
            conexion.Open()
